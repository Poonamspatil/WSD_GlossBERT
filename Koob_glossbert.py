...
def construct_context_gloss_pairs_through_nltk(input, target_start_id, target_end_id, gloss_dict):
    """
    construct context gleoss pairs like sent_cls_ws
...
def remove_punctuation(input_string, punc1, punc2):
    """
    Description: Remove/ replace unnecessary punctuations from ontext sentence

    Input:
        input_string: (str), input string/ token
        punc1: (str), punctuations to replace
        punc2: (str), punctuations to remove

    Output:
         input_string : (str), input string without punctuation
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", default="./request.json", type=str)
    args = parser.parse_args()

    request_file = open(args.input, 'r')
    ...
    result_json = []
    for item in jsonobj[1:5]:
        word_id, word, teacher_vote, ai_vote = process_input_json(item)

        result_dict = {'word_id': word_id, 'word': word, 'Teacher_vote': teacher_vote, 'AI_vote': ai_vote}
